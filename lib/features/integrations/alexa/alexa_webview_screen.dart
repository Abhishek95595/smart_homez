...
class _AlexaWebViewScreenState extends State<AlexaWebViewScreen> {
  ...
  String? _lastUrl;
  bool _returningCallback = false;

  @override
  void initState() {
    super.initState();

    final Uri effectiveUri = widget.authorizeUri;
          ...
          onNavigationRequest: (NavigationRequest request) {
            ...
            final bool isCallback =
                ...
                uri.scheme.toLowerCase() ==
                    widget.redirectScheme.toLowerCase() &&
                uri.host.toLowerCase() == 'alexa-callback';
            if (isCallback) {
              debugPrint(
                '[AlexaWebView] Intercepted callback / custom scheme: $url',
              );
              if (mounted && !_returningCallback) {
                _returningCallback = true;
